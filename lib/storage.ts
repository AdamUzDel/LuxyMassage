...
export class StorageService {
  ...
  async uploadProviderImage(providerId: string, file: File, isServer = false): Promise<UploadResult> {
    ...
    try {
      ...
      // Get public URLs
      const { data: urlData } = this.supabase.storage.from("provider-images").getPublicUrl(fileName)

      const { data: thumbnailUrlData } = thumbnailData
        ? this.supabase.storage.from("provider-images").getPublicUrl(thumbnailName)
        : { data: null }
      ...
      if (dbError) {
        ...
        await this.supabase.storage.from("provider-images").remove([fileName])
        if (thumbnailData) {
          await this.supabase.storage.from("provider-images").remove([thumbnailName])
        }
  ...
  async deleteProviderImage(imageId: string, isServer = false): Promise<void> {
    ...
    try {
      ...
      const { error: storageError } = await this.supabase.storage.from("provider-images").remove(filesToDelete)
  ...
  async updateImageOrder(imageId: string, sortOrder: number, isServer = false): Promise<void> {
    ...
    try {
      ...
      // Update sort order
      const { error } = await this.supabase.from("provider_images").update({ sort_order: sortOrder }).eq("id", imageId)
